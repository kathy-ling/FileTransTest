...
public:
	...
	void UnzipFile(CString strFilePath, CString strTempPath);
...
protected:
    ...
    afx_msg void OnStopAccept();
    afx_msg void OnStartAccept();
	void OnStartListening();
    //}}AFX_MSG
    DECLARE_MESSAGE_MAP()
private:
    void DispInfo(CString &strInfo);
    void DispNetMsg(CString &strNetMsg);
    ...
    static void OnSocketClose(void *pNotifyObj, SOCKET hSocket, EMSocketCloseReason scr);
    static void OnOneNetMsg(void *pNotifyObj, char *Msg, int nMsgLen);
    static void OnRecvFileRequest(void *pNotifyObj, char *szPathName, BOOL &bRecv);
    ...
    static void OnRecvFileFail(void *pNotifyObj, char *szPathName, EMRecvFileFailReason RecvFilFailReason);
};
