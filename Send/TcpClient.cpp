...
ReCallFunForTcpClient pReCallFunForTcpClient = NULL;
...
void CTcpClient::SetOnSendFileRate(SendFileRateFun OnSendFileRate)
{
	m_OnSendFileRate = OnSendFileRate;
}
//  初始化windows socket
...
LRESULT CTcpClient::HideWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SOCKETSENDERR: // socket发送出错
        ...
        //AfxMessageBox("222");
        ///VTR
        //if (NULL != pReCallFunForTcpClient)
        //{
        //    pReCallFunForTcpClient();
        //}
		break;
	...
	case WM_SENDFILEPROGRESS: // 文件发送进度
		...
		break;
	case WM_SENDFILERATE: // 文件发送速率
		pTcpClient->HandleSendFileRateMsg();
		break;
	case WM_WAITTIMEOUT:
...
void CTcpClient::HandleSendFileProgressMsg(void)
{
	...
	//////时间及计算方法
		...
		dwSentRate=  m_pSendFileThread->GetSentRate();
		m_OnSendFileProgress(m_pNotifyObj, dwSentBytes, dwFileSize);
		m_OnSendFileRate(m_pNotifyObj, dwSentRate);
...
// 处理文件发送速率窗体消息
void CTcpClient::HandleSendFileRateMsg(void)
{
	DWORD dwSentRate ; // 已发送速率

	if(!IsSending())
		return;

	if(m_OnSendFileRate != NULL)
	{
		dwSentRate = m_pSendFileThread->GetSentRate();
		m_OnSendFileRate(m_pNotifyObj, dwSentRate);
	}
...
DWORD CSendFileThread::GetSentRate(void)
{
	return m_dwSentRate;
}
...
void CSendFileThread::SendFileData(void)
{
	...
	while(!m_bTerminated && m_dwSentBytes < m_dwFileSize)
	{
		...
		{
			m_dwSentRate = nBytes/(dwTickCount )*1000;
